  ...
  methods: {
    ...
    async scanFolder(){
      ...
      try{
        // 构建映射表对象
        const mapObj={};
        for(const row of this.mapRows){if(row.zh&&row.en)mapObj[row.zh]=row.en;}
        const r=await fetch('/api/label/scan',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({folder:this.folder,trigger:this.trigger,map:mapObj})});
        const d=await r.json();
